...
| `FC-003` | Source runs install as a distinct `OpenChamber CUSTOM` application on Linux and macOS, with silent non-HMR launchers that rebuild bundled UI assets and write the current run log to a file. | Allow the official packaged application and source fork to be pinned separately while preventing source edits from reloading the launched app and preserving startup failures and runtime logs without opening a terminal. | `package.json`, `scripts/install-custom.sh`, `packages/electron/main.mjs`, `packages/electron/scripts/electron-dev.mjs`, `packages/electron/scripts/launch-custom-linux.sh`, `packages/electron/README.md`, `docs/fork-runtime-state.md` | Remove if upstream supports configurable development desktop identity and a logged non-HMR source launcher with equivalent behavior. |
