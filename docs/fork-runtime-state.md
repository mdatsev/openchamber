...
The external OpenCode service selects an installed shell launcher that preserves the original command for OpenCode parsing and permission approval, then executes it in a transient `opencode-tools.slice` scope. Each command defaults to `MemoryMax=3G`, `MemorySwapMax=512M`, and `TasksMax=512`; the aggregate slice defaults to `MemoryMax=7G`, `MemorySwapMax=1G`, and `TasksMax=2048`. The OpenCode service itself uses `MemoryMax=9G`, `MemorySwapMax=2G`, `OOMPolicy=continue`, and `ManagedOOMPreference=avoid`. OpenChamber services use `MemoryMax=3G`, `MemorySwapMax=512M`, and `OOMPolicy=kill`. Lower `MemoryHigh` thresholds are intentionally omitted because an anonymous-memory allocation can remain throttled at the soft boundary indefinitely instead of reaching `MemoryMax` and returning an OOM failure. Override provider-specific environment variables in mode-0600 `~/.config/openchamber/opencode.env` rather than adding secrets to unit files.
