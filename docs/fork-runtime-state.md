...
**Pull and build latest custom** is a guarded deployment preparation rather than generic `git pull`. It requires the live checkout to remain on a fully clean `custom` branch and to be an ancestor of the fetched target. The runtime creates a detached staging worktree beside the live checkout, runs the target lockfile install and web build there, validates `index.html` and non-empty assets, and retains existing content-hashed chunks for already-loaded clients. It then writes a mode-0600 prepared-update record under the installed runtime directory and leaves live source, dependencies, and assets unchanged. Settings continues to show the old running and checked-out commits plus the prepared target until the user explicitly restarts the interface.
