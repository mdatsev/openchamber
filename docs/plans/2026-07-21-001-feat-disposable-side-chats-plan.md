...
- **Use an OpenChamber-owned lifecycle operation around the official fork API.** The shared server route resolves the requested context boundary, creates the upstream fork, marks it as disposable, and deletes it if marking fails. The UI captures the active runtime transport for operation identity while reusing shared browser-header sanitization; official session message, abort, update, and delete operations continue through `opencodeClient`.
...
flowchart TB
  ...
  B -->|captured runtime transport| C[OpenChamber side-chat route]
