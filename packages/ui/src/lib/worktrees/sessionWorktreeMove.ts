...
const moveSessionTreeToQuickWorktree = async (input: {
  ...
  sourceDirectory: string;
  moveChanges: boolean;
}, target?: SessionWorktreeMoveTarget): Promise<string> => {
  return moveSessionTreeTransaction(input, async () => {
    ...
    const worktreeOptions = target
      ? { ...(sourceBranch ? { startRef: sourceBranch } : {}), ...normalizeSessionWorktreeMoveTarget(target) }
      : sourceBranch ? { startRef: sourceBranch } : {};
...
const executeSessionTreeWorktreeMove = (
  ...
  setSessionMoveRequesting(input.root.id, false);
...
export const startSessionTreeWorktreeMove = (input: SessionTreeWorktreeMoveInput): void => {
  const state = useSessionMoveState.getState();
  if (state.pendingSessionIds.has(input.root.id) || state.requestingSessionIds.has(input.root.id)) return;
  setSessionMoveRequesting(input.root.id, true);

  void (async () => {
    try {
      const isGitRepository = await checkIsGitRepository(input.sourceDirectory);
      if (!isGitRepository) {
      ...
      useSessionMoveState.setState((current) => ({
        confirmationQueue: [...current.confirmationQueue, request],
      }));
    } catch (error) {
      setSessionMoveRequesting(input.root.id, false);
...
export const resolveSessionWorktreeMoveConfirmation = (
  ...
  if (choice === null) {
    setSessionMoveRequesting(request.input.root.id, false);
    return;
  }
