...
import { canShowSessionWorktreeMenu, getSessionWorktreeMenuDisabled, nodeContainsSessionId, nodeHasPinnedMembershipChange, selectRowBadgeVisibilityClass } from './sessionNodeItemUtils';
...
import type { SortableDragHandleProps } from '../projects/sortableItems';
import {
...
function SessionNodeItemComponent(props: SessionNodeItemProps): React.ReactNode {
  ...
  const sessionTitle = session.title || t('sessions.sidebar.session.untitled');
  ...
  // SAFETY: OpenCode session payloads may include the parent ID used to classify subtasks.
  ...
  const isMultiRunLikeSession = React.useMemo(() => parseMultiRunSessionTitle(session.title) !== null, [session.title]);
  ...
  const doExportSession = React.useCallback(async (includeSubtasks: boolean) => {
    ...
    const markdown = formatSessionAsMarkdown(records, session.title ?? null, childExports);
    ...
    toast.success(t('sessions.sidebar.session.export.success'));
    showSkippedSubtasksWarning(skippedSubtaskCount);
  }, [collectChildExports, loadExportRecords, node.children, session.id, session.title, sessionDirectory, showSkippedSubtasksWarning, t]);
  ...
  const renderSessionMenuItems = ({
    ...
    <>
      ...
      {!session.share ? (
        <Item onClick={() => handleShareSession(session)} className="[&>svg]:mr-1">
          <Icon name="share-2" className="mr-1 h-4 w-4" />
          {t('sessions.sidebar.session.menu.share')}
        </Item>
      ) : (
        <>
          <Item onClick={() => { if (session.share?.url) handleCopyShareUrl(session.share.url, session.id); }} className="[&>svg]:mr-1">
                ...
                <TooltipTrigger asChild>
                  ...
                  >
                    <div className="flex w-full items-center min-w-0 flex-1 gap-1 overflow-hidden">
                      ...
                      <span className={badgeVisibilityClass}>
...
const sessionNodeItemPropsChange = (prev: SessionNodeItemProps, next: SessionNodeItemProps): string | null => {
  ...
  if (prev.worktreeDragHandleProps !== next.worktreeDragHandleProps) return 'worktreeDragHandleProps';
