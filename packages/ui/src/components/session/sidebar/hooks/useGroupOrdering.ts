...
export const useGroupOrdering = (groupOrderByProject: Map<string, string[]>) => {
  const getOrderedGroups = React.useCallback(
    (projectId: string, groups: SessionGroup<CatalogSessionNode>[]) => {
      const preferredOrder = groupOrderByProject.get(projectId);
      if (!preferredOrder || preferredOrder.length === 0) {
        return groups;
      }
      const groupById = new Map(groups.map((group) => [group.id, group]));
      const ordered: SessionGroup<CatalogSessionNode>[] = [];
      ...
      const newGroups: SessionGroup<CatalogSessionNode>[] = [];
      const trailingGroups: SessionGroup<CatalogSessionNode>[] = [];
