...
- Mounted worktree headers may perform one summary-only comparison request per directory. Store-level in-flight deduplication coalesces duplicate desktop/mobile consumers, and successful mutating tools, file saves, session reverts, and integration actions provide refresh hints instead of enabling a timer.
