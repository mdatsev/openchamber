...
- Session rows have a single layout (former `minimal`); the `default`/`minimal` display mode was removed (`session-display-mode` store v4 migration drops the key). Rows show an inline branch label (from `node.worktree` or recent's `secondaryMeta`) when the session lives outside the project root, and bold titles while unread. Authoritative pending-question state displays the question icon ahead of `busy`/`retry`, including after bootstrap or reconnect. A materialized unfinished turn omitted from a later authoritative active-status snapshot displays a static warning icon rather than a busy spinner.
...
- Prime Agent root sessions use the same compact row geometry and no separate section heading; a harness glyph and Prime-owned working/read-only state distinguish them. Their authoritative `directory` is projected into the nearest registered project/worktree for presentation on desktop and mobile, while unmatched sessions remain in the top flow. Prime roots also participate in the 48-hour/working Recent presentation. Durable Prime child sessions render recursively beneath their parent, expanded by default with a local disclosure toggle; selecting one opens its independently attached transcript. This projection never inserts Prime data into OpenCode project ownership, sync, folders, pinning, search, prefetch, or session actions. Selecting a projected row activates its project presentation without changing `currentSessionId` or starting OpenCode directory bootstrap. The active-branch transcript exposes Prime-owned send/abort, model, thinking-level, and discovered slash-command controls only while the adapter reports interactive readiness; unavailable sessions retain read-only transcript access. The runtime-scoped catalog refreshes on visible mount, window focus, and Prime lifecycle events, preserves its last successful same-runtime result on refresh failure, and stays hidden when Prime state is absent or the runtime explicitly reports unsupported.
...
- `../PrimeSessionsSection.tsx`: Prime Agent rows with shared sidebar geometry and adapter-owned activity, isolated from OpenCode session data, menus, grouping mutations, and prefetch.
- `../usePrimeSessionsCatalog.ts`: Runtime-scoped Prime catalog loading, partial-result reconciliation, focus refresh, and lifecycle-event activity updates.
...
- `sessionOwnership.ts`: Resolves directories once into project/worktree locations. OpenCode sessions use the resulting ownership and folder-scope indexes; Prime sessions reuse only the directory-to-location projection.
