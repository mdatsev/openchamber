...
- Session rows have a single layout (former `minimal`); the `default`/`minimal` display mode was removed (`session-display-mode` store v4 migration drops the key). Rows show an inline branch label (from `node.worktree` or recent's `secondaryMeta`) when the session lives outside the project root, and bold titles while unread. Authoritative pending-question state from the session or any descendant subagent displays the question icon ahead of `busy`/`retry`, including after bootstrap or reconnect. A materialized unfinished turn omitted from a later authoritative active-status snapshot displays a static warning icon rather than a busy spinner.
