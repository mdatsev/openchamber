...
- Root session menus can quickly create a worktree from the session directory's current branch and move the full session subtree there while idle. The browser-local Custom Fork preference controls whether checkout changes move with the root session; rollback uses the same captured preference as the forward move.
