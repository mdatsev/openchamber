...
- The actual project zones, Recent zone, and search results include passive Prime/RLM history alongside OpenCode sessions. Prime roots use authoritative configured-project/worktree ownership; descendants inherit the validated root owner, while missing/cyclic/mismatched ancestry remains omitted.
...
- When sticky zone headers are enabled, project headers are sticky "zone" bands (`SortableProjectItem`); on a vibrant desktop the scrolling content fades behind an unmasked, non-interactive copy of the stuck icon/title without painting a background. The transparent fade zone blocks interaction with obscured rows. The `recent` section uses the same overlay while it is the leading sticky header. Collapsed projects show an aggregated busy/unseen indicator (`ProjectAggregateStatusIndicator`), derived from the live status index, notification store, and pending questions scoped to the project's directories. A pending question uses the question icon and takes precedence over a busy spinner.
- Session rows have a single layout (former `minimal`); the `default`/`minimal` display mode was removed (`session-display-mode` store v4 migration drops the key). Rows show an inline branch label (from `node.worktree` or recent's `secondaryMeta`) when the session lives outside the project root, and bold titles while unread. All desktop, switcher, and mobile rows derive active styling from the shared visible chat identity rather than independently reading an adapter's private selection. Authoritative pending-question state displays the question icon ahead of `busy`/`retry`, including after bootstrap or reconnect. A materialized unfinished turn omitted from a later authoritative active-status snapshot displays a static warning icon rather than a busy spinner.
...
- `sessionOwnership.ts`: Owns the generic root-ownership engine plus the OpenCode SDK wrapper. It resolves root-session directories once into shared project/worktree ownership and folder-scope indexes. Only roots are assigned directly; descendants inherit their valid root owner, while missing, cyclic, or archive-inconsistent ancestry is omitted rather than promoted.
- `sessionCatalog.ts`: Defines the SDK-free, harness-neutral catalog identity, timestamps, directory ownership, graph, and revision. Sidebar search synchronously filters the current projected graph.
- `openCodeSessionAdapter.ts`: Converts genuine OpenCode SDK sessions into the neutral catalog, privately retains SDK records/worktree metadata, and projects validated nodes into the cohesive OpenCode row controller.
- `primeSessionAdapter.ts`: Owns Prime working-directory privacy, valid-root ownership projection, opaque project/worktree group placement, the passive Prime navigation controller, and the private directory lookup consumed by the focused visible-chat directory hook.
- `sessionPresentationAdapter.ts`: Projects a merged harness-neutral graph into controller-backed sidebar nodes at the single harness dispatch boundary.
...
- Catalog ancestry is closed before presentation. Project/worktree assignment evaluates root sessions only; descendants inherit the root owner. A descendant whose parent is missing, whose ancestry cycles, or whose archived state conflicts with its root is omitted from sidebar, Recent, search, and mobile/switcher projections and is never promoted into a synthetic root.
- `SessionCatalogEntry`, `SessionCatalogNode`, and `CatalogSessionNode` are harness-neutral and SDK-free. OpenCode and passive Prime catalogs are merged before project grouping, Recent membership, or synchronous search filtering, so every desktop/mobile sidebar projection derives from the same closed graph. Adapter-private working directories resolve roots to opaque group IDs and never enter catalog entries, nodes, controllers, or graph revisions. The focused visible-chat directory hook may project the selected Prime record's private directory to explicitly directory-scoped panels; if that placement is absent, it returns no directory rather than falling back to a stale OpenCode selection.
- `SessionNodeItem` is the neutral row/controller boundary. Both OpenCode and passive Prime controllers render through the same `SessionRowPresentation` frame, show their shared harness marker inside the title/metadata flex (where row action padding can reserve space), and select by complete chat identity. Any Prime identity selection, including a passive deep link, closes an open new-session draft before publishing the visible session; only the explicit user-selection hook adds live activation. The OpenCode controller privately recovers genuine SDK sessions for rename/share/archive/delete/pin/folder/worktree/live behavior; the Prime controller exposes navigation and catalog activity only, so mutating menu actions are absent rather than disabled or simulated. The legacy SDK `SessionNode` type remains only as a private OpenCode-controller compatibility contract.
