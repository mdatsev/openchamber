...
- Layout (web/desktop): top navigation (`SidebarNav`: New session, Scheduled, Multi-run, Archive), then the `recent` zone, then one zone per project. The selected grouping mode controls whether sessions render as a merged flat list or under compact worktree-aware presentation.
...
- When sticky zone headers are enabled, project headers are sticky "zone" bands (`SortableProjectItem`); on a vibrant desktop the scrolling content fades behind an unmasked, non-interactive copy of the stuck icon/title without painting a background. The transparent fade zone blocks interaction with obscured rows. The `recent` section uses the same overlay while it is the leading sticky header. Collapsed projects show an aggregated busy/unseen indicator (`ProjectAggregateStatusIndicator`), derived from the live status index, server-backed session inbox, and pending questions scoped to the project's directories. A pending question uses the question icon and takes precedence over a busy spinner.
- Session rows have a single layout (former `minimal`); the `default`/`minimal` display mode was removed (`session-display-mode` store v4 migration drops the key). Rows show an inline branch label (from `node.worktree` or recent/group `secondaryMeta`) when the session lives outside the project root, and bold titles while unread. Linked-worktree rows also show a muted worktree glyph followed by a success check for an available clean primary-branch comparison or a warning commit glyph for an available changed comparison; loading, failed, and unavailable comparisons infer no clean state. Session menus expose server-backed Pin/Unpin and Mark as read/Mark as unread actions; the mobile sessions sheet exposes the same actions in its swipe tray. Authoritative pending-question state from the session or any descendant subagent displays the question icon ahead of `busy`/`retry`, including after bootstrap or reconnect. A materialized unfinished turn omitted from a later authoritative active-status snapshot displays a static warning icon rather than a busy spinner. Running and persistent error states remain separate from unread completion state.
- Project headers show a warning-colored file-edit indicator only when the root project's authoritative Git status reports uncommitted changes. Worktree dirty state is intentionally represented by the linked-worktree comparison indicators instead.
...
- `SidebarProjectsList.tsx`: Main scrollable renderer for project zones and their flat/archived groups plus empty/search states; owns project and worktree drag-to-reorder and root-project Git status presentation.
- `SessionGroupSection.tsx`: Renders one flat (or archived) group: sessions first, then flat folder entries with path labels, show-more batching, and explicit loading/error/retry state for empty groups. In by-worktree mode it also selects compact zero/one-conversation presentation without changing the underlying group or drag identity. Archived buckets (VS Code) virtualize past 50 rows.
- `WorktreeChangesIndicator.tsx`: Displays a warning-colored commit marker when a cached linked-worktree comparison differs from the branch currently checked out in the primary worktree. Mobile headers own their mounted request/listener; the desktop sidebar passes centralized refresh ownership. Neither path polls.
...
- The visible desktop sidebar centralizes one summary-only comparison request per linked-worktree directory and one Git refresh-hint listener; rows and worktree headers consume leaf cache entries without mounting their own listeners. Store-level in-flight deduplication still coalesces desktop/mobile consumers. Successful mutating tools, file saves, session reverts, and integration actions provide refresh hints instead of enabling a timer; root-directory hints also refresh the project dirty indicator.
