...
- Root session menus can quickly create a worktree from the session directory's current branch and move the full session subtree there while idle. A clean checkout moves immediately; a dirty checkout first asks whether to move its uncommitted changes or leave them in the source checkout. Rollback uses the same choice as the forward move.
...
- `SessionWorktreeMoveDialog.tsx`: Shared dirty-checkout choice for web, Desktop, hosted mobile, and Capacitor session-to-worktree moves.
