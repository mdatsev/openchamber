...
export const UserMessageContent = React.memo(({ messageId, parts, isMobile, agentMention }: {
    messageId: string;
    parts: Part[];
    isMobile: boolean;
    agentMention?: AgentMentionInfo;
    ...
    const userContentParts = React.useMemo(() => {
        return parts.filter((part) => {
            if (part.type === 'text') return !isEmptyTextPart(part);
...
export const UserMessageAttachments = React.memo(({ parts, onShowPopup }: {
    parts: Part[];
    onShowPopup: (content: ToolPopupContent) => void;
    ...
    if (isUser) {
        return (
            <>
                ...
                />
                <UserMessageAttachments parts={props.parts} onShowPopup={props.onShowPopup} />
            </>
        );
    }
