...
- Tool-call patch previews use word-level intraline highlighting for paired changed lines up to 1,000 characters. When Pierre skips a longer pair, both affected lines retain whole-line highlighting and show an accessible warning marker; this does not change the separate performance policy for full-file diff views.
