...
export const StatusRow: React.FC<StatusRowProps> = ({
  ...
  const shouldRenderPlaceholder = !abortActive && !isInterrupted && !wasAborted;
  const hasContent = isWorking || Boolean(isInterrupted) || Boolean(wasAborted);
    ...
    >
      ...
      <div className="oc-glass-popover inline-flex w-max max-w-full items-center gap-2 h-8 whitespace-nowrap rounded-full [corner-shape:round] px-3">
        <div className="flex items-center min-w-0 gap-2 overflow-x-hidden">
          {wasAborted ? (
            <span className="flex items-center gap-1.5 typography-ui-label text-[var(--status-error)]">
              <Icon name="close-circle" className="size-3.5" aria-hidden="true" />
              {t('chat.statusRow.aborted')}
            </span>
          ) : isInterrupted ? (
            <span className="flex items-center gap-1.5 typography-ui-label text-status-warning" role="status" aria-live="polite">
