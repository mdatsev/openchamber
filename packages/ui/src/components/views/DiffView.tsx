...
import { branchRangeKey, isBranchScopeAvailable, isBranchScopeDefinitelyUnavailable, useRangeKeyedCache, useBoundedDirectoryRetry } from './branchDiffScope';
import { getBranchBase, getGitRangeDiff, getGitRangeFiles } from '@/lib/gitApi';
import { getRuntimeKey } from '@/lib/runtime-switch';
import { cn } from '@/lib/utils';
import type { GitStatus, GitRangeFileEntry, GitWorktreeComparisonMode } from '@/lib/api/types';
import {
    DropdownMenu,
    DropdownMenuContent,
...
import { mapWorktreeComparisonGroups } from '@/lib/git/worktreeComparison';
...
const getFileEntryKey = (file: FileEntry): string => file.comparisonKey ?? file.path;
...
type DiffScope = 'all' | 'branch' | 'branch-range' | 'staged' | 'working' | 'turn';
type PersistedDiffScope = Exclude<DiffScope, 'all' | 'branch-range'>;

const getComparisonModeLabelKey = (mode: GitWorktreeComparisonMode): I18nKey => {
    if (mode === 'uncommitted') return 'diffView.comparison.mode.uncommitted';
    if (mode === 'committed') return 'diffView.comparison.mode.committed';
    return 'diffView.comparison.mode.combined';
};
...
interface ChangeScopeSelectorProps {
    ...
    onScopeChange?: (scope: Exclude<DiffScope, 'all'>) => void;
}
...
const ChangeScopeSelector = React.memo<ChangeScopeSelectorProps>(({
    ...
    switch (scope) {
        case 'branch':
            currentCount = worktreeCount;
            currentLabel = t('diffView.scope.branchAgainst', { branch: worktreeBaseBranch || '' });
            break;
        case 'branch-range':
            currentCount = branchCount ?? 0;
            currentLabel = t('diffView.scope.branch');
            break;
        case 'staged':
            currentCount = stagedCount;
            currentLabel = t('diffView.scope.staged');
            break;
        case 'turn':
            currentCount = turnCount;
            currentLabel = t('diffView.scope.lastTurn');
            break;
        default:
            currentCount = workingCount;
            currentLabel = t('diffView.scope.changed');
    }
        ...
        <DropdownMenu open={open} onOpenChange={setOpen}>
            ...
            <DropdownMenuContent align="start" className="w-40">
                <DropdownMenuRadioGroup
                    value={scope}
                    onValueChange={(value) => {
                        if (value === 'branch' || value === 'branch-range' || value === 'working' || value === 'staged' || value === 'turn') {
                            onScopeChange?.(value);
                            setOpen(false);
                        }
                    }}
                >
                    {worktreeAvailable ? (
                        <DropdownMenuRadioItem value="branch">
                            <span className="flex min-w-0 flex-1 items-center justify-between gap-3">
                                <span>{t('diffView.scope.branchAgainst', { branch: worktreeBaseBranch || '' })}</span>
                                <span className="typography-meta text-muted-foreground">{worktreeCount}</span>
                            </span>
                        </DropdownMenuRadioItem>
                    ) : null}
                    ...
                    {showBranchOption ? (
                        <DropdownMenuRadioItem value="branch-range">
                            <span className="flex min-w-0 flex-1 items-center justify-between gap-3">
        ...
        <ScrollableOverlay outerClassName="flex-1 min-h-0" className="px-2 py-2">
            <ul className="flex flex-col gap-1">
                {changedFiles.map((file) => {
                    ...
                    const isActive = selectedFile === getFileEntryKey(file);

                    return (
                            <li key={getFileEntryKey(file)}>
                            <button
                                type="button"
                                    onClick={() => onSelectFile(getFileEntryKey(file))}
                                className={cn(
...
const MultiFileDiffEntry = React.memo<MultiFileDiffEntryProps>(({
    ...
    const setSectionRef = React.useCallback((node: HTMLDivElement | null) => {
        ...
        registerSectionRef(entryKey, node);
    }, [entryKey, registerSectionRef]);

    const handleOpenChange = React.useCallback((open: boolean) => {
        onExpandedChange(entryKey, open);
    }, [entryKey, onExpandedChange]);
    ...
    const handleFileAction = React.useCallback(async (action: FileDiffAction) => {
        ...
        try {
            ...
            useGitStore.getState().invalidateWorktreeComparison(directory);
    ...
    return (
        <div ref={setSectionRef} className="scroll-mt-9 border-b border-[var(--interactive-border)]/40 last:border-b-0">
            {file.comparisonSectionLabel ? (
                <div className="border-b border-[var(--interactive-border)]/35 bg-[var(--surface-muted)] px-3 py-1.5 typography-micro font-semibold text-muted-foreground">
                    {file.comparisonSectionLabel}
                </div>
            ) : null}
...
interface DiffViewProps {
    ...
    showOpenInEditorAction?: boolean;
    diffScope?: DiffScope;
    onDiffScopeChange?: (scope: PersistedDiffScope) => void;
...
export const DiffView: React.FC<DiffViewProps> = ({
    ...
    const setDiff = useGitStore((state) => state.setDiff);
    const [comparisonMode, setComparisonMode] = React.useState<GitWorktreeComparisonMode>('combined');
    const worktreeComparison = useWorktreeComparisonFull(effectiveDirectory ?? null, comparisonMode);
    const worktreeComparisonError = useWorktreeComparisonError(effectiveDirectory ?? null, comparisonMode);
    const isLoadingWorktreeComparison = useWorktreeComparisonLoading(effectiveDirectory ?? null, comparisonMode);
    ...
    const worktreeComparisonEntries = React.useMemo(() => mapWorktreeComparisonGroups(worktreeComparison)
        .flatMap((group) => group.entries.map((entry, index) => ({
            ...entry,
            comparisonSectionLabel: index === 0
                ? group.kind === 'committed'
                    ? t('diffView.comparison.layer.committed')
                    : group.kind === 'staged'
                        ? t('diffView.comparison.layer.staged')
                        : group.kind === 'unstaged'
                            ? t('diffView.comparison.layer.unstaged')
                            : group.kind === 'untracked'
                                ? t('diffView.comparison.layer.untracked')
                                : t('diffView.comparison.layer.combined')
                : undefined,
        }))), [t, worktreeComparison]);
    const worktreeComparisonDiffData = React.useMemo(() => {
        const entries = new Map<string, DiffData>();
        for (const entry of worktreeComparisonEntries) {
            entries.set(
                entry.comparisonKey,
                entry.patch
                    ? createTextDiffDataFromPatch(entry.path, entry.patch, 'patch')
                    : { original: '', modified: '', contextMode: 'patch' },
            );
        }
        return entries;
    }, [worktreeComparisonEntries]);
    const worktreeComparisonAvailable = worktreeComparison?.available === true;
    const worktreeComparisonSelected = Boolean(git.getWorktreeComparison)
        && (activeDiffScope === 'all' || activeDiffScope === 'branch');
    let renderedDiffScope: DiffScope = activeDiffScope;
    if (activeDiffScope === 'all' && worktreeComparisonAvailable) {
        renderedDiffScope = 'branch';
    } else if (
        activeDiffScope === 'branch'
        && (!git.getWorktreeComparison || (worktreeComparison !== null && !worktreeComparisonAvailable))
    ) {
        renderedDiffScope = 'working';
    }

    let forcedStaged: boolean | null = null;
    if (renderedDiffScope === 'staged') forcedStaged = true;
    else if (renderedDiffScope === 'working') forcedStaged = false;
    const activeDiffStaged = forcedStaged ?? displayFileStaged;

    const isMobileLayout = isMobile || screenWidth <= 768;
    const isWorktreeComparisonScope = renderedDiffScope === 'branch';
    const showReviewAction = Boolean(currentSessionId) && renderedDiffScope !== 'turn' && !isWorktreeComparisonScope && !isMobileLayout && !isVSCodeRuntime();
    // Same runtime and width rules as the rail surface: no point offering an
    // entry point to a surface that cannot open here.
    const showWalkthroughAction = renderedDiffScope !== 'turn' && !isWorktreeComparisonScope && !isMobileLayout && !isVSCodeRuntime();
    ...
    // The upstream branch-range capability is local to this view so the fork's
    // persisted `branch` scope keeps its linked-worktree meaning. Coerce the
    // range only after metadata confirms it is unavailable.
    React.useEffect(() => {
        if (activeDiffScope === 'branch-range' && branchScopeDefinitelyUnavailable) {
    ...
    React.useEffect(() => {
        if (activeDiffScope === 'branch-range') {
            reloadBranchFiles();
        }
    ...
    const branchDiffRangeKey = activeDiffScope === 'branch-range' && effectiveDirectory && currentBranch && branchBase
        ? branchRangeKey(effectiveDirectory, branchBase, currentBranch)
        : null;
    const branchDiffPathsKey = React.useMemo(
        () => (activeDiffScope === 'branch-range' ? Array.from(expandedFiles).sort().join('\0') : ''),
    ...
    const changedFiles: FileEntry[] = React.useMemo(() => {
        if (renderedDiffScope === 'branch') {
            return worktreeComparisonEntries;
        }

        if (renderedDiffScope === 'branch-range') {
            return (branchFiles ?? [])
                .map((file) => ({
                    path: file.path,
                    index: '',
        ...
        const includeFile = renderedDiffScope === 'staged'
            ? isStagedStatusFile
            : renderedDiffScope === 'working'
                ? isWorkingStatusFile
                : () => true;
        ...
        return status.files
            ...
            .sort((a, b) => a.path.localeCompare(b.path));
    }, [branchFiles, lastTurnDiffs, renderedDiffScope, status, worktreeComparisonEntries]);

    const worktreeFileCount = worktreeComparisonEntries.length;

    const changedFilePathsKey = React.useMemo(
        () => changedFiles.map(getFileEntryKey).join('\0'),
        [changedFiles],
    );
    ...
    React.useEffect(() => {
        ...
        const scopeKey = `${effectiveDirectory ?? ''}:${renderedDiffScope}:${stackedDefaultCollapsedAll ? 'collapsed' : 'default'}`;
    ...
    }, [changedFilePathsKey, effectiveDirectory, renderedDiffScope, stackedDefaultCollapsedAll]);
    ...
    React.useEffect(() => {
        if (!isActive || !effectiveDirectory || isGitRepo !== true || !git.getWorktreeComparison) return;
        void ensureWorktreeComparison(effectiveDirectory, git, {
            mode: comparisonMode,
            includePatches: true,
            contextLines: DEFAULT_CONTEXT_DIFF_LINES,
        });
    }, [comparisonMode, effectiveDirectory, ensureWorktreeComparison, git, isActive, isGitRepo]);
    ...
    React.useEffect(() => {
        ...
        return sessionEvents.onGitRefreshHint((hint) => {
            ...
            const linkedDirectory = normalizePath(effectiveDirectory);
            const primaryDirectory = worktreeComparison?.available
                ? normalizePath(worktreeComparison.primaryWorktree)
                : '';
            if (hintDirectory !== linkedDirectory && hintDirectory !== primaryDirectory) {
                return;
            }
            if (hintDirectory === linkedDirectory && hint.paths?.length) {
            ...
            if (hintDirectory === linkedDirectory) {
                void fetchStatus(effectiveDirectory, git, { silent: true });
            }
            if (isActive && isGitRepo === true && git.getWorktreeComparison) {
                void fetchWorktreeComparison(effectiveDirectory, git, {
                    mode: comparisonMode,
                    includePatches: true,
                    contextLines: DEFAULT_CONTEXT_DIFF_LINES,
                });
    ...
    }, [captureScrollAnchor, clearDiffCache, comparisonMode, effectiveDirectory, fetchStatus, fetchWorktreeComparison, git, isActive, isGitRepo, worktreeComparison]);
    ...
    React.useEffect(() => {
        ...
        const stillExists = changedFiles.some((file) => getFileEntryKey(file) === displayFile);
        if (!stillExists) {
            setDisplayFile(null);
    ...
    const handleExpandOrCollapseAll = React.useCallback(() => {
        ...
        setExpandedFiles((previous) => {
            ...
            return new Set(changedFiles.map(getFileEntryKey));
        });
                ...
                <div className="relative flex-1 min-w-0 min-h-0 h-full">
                    ...
                    >
                        <div className="flex flex-col [overflow-anchor:none]" data-diff-virtual-content>
                            {changedFiles.map((file) => (
                                <MultiFileDiffEntry
                                    key={`${getFileEntryKey(file)}:${fileDiffRefreshNonce.get(file.path) ?? 0}`}
                                    entryKey={getFileEntryKey(file)}
                                    directory={effectiveDirectory}
                                    file={file}
                                    layout={getLayoutForFile(file)}
                                    wrapLines={diffWrapLines}
                                    isSelected={displayFile === getFileEntryKey(file)}
                                    isExpanded={expandedFiles.has(getFileEntryKey(file))}
                                    isMounted={mountedStackedFiles.has(getFileEntryKey(file)) || getFileEntryKey(file) === pinnedStackedTarget}
                                    onSelect={handleSelectFile}
                                    onExpandedChange={handleStackedEntryExpandedChange}
                                    registerSectionRef={registerSectionRef}
                                    showOpenInEditorAction={showOpenInEditorAction && renderedDiffScope !== 'turn'}
                                    ...
                                    readOnly={file.readOnly || renderedDiffScope === 'branch-range'}
                                    loadFullFiles={isWorktreeComparisonScope ? false : loadFullFiles}
                                    initialDiffData={renderedDiffScope === 'branch'
                                        ? worktreeComparisonDiffData.get(getFileEntryKey(file)) ?? null
                                        : renderedDiffScope === 'turn'
                                            ? lastTurnDiffData.get(file.path) ?? null
                                            : renderedDiffScope === 'branch-range'
    ...
    const renderContent = () => {
        ...
        if (renderedDiffScope !== 'turn' && isLoadingStatus && !status) {
            return (
        ...
        if (renderedDiffScope !== 'turn' && isGitRepo === false) {
            return (
                <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        ...
        if (worktreeComparisonSelected && !worktreeComparison && !worktreeComparisonError) {
            return (
                <div className="flex flex-1 items-center justify-center gap-2 text-sm text-muted-foreground">
                    <Icon name="loader-4" className="size-4 animate-spin" />
        ...
        if (changedFiles.length === 0) {
            return (
                <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
                    {renderedDiffScope === 'branch' && worktreeComparison?.available
                        ? comparisonMode === 'uncommitted'
                            ? t('diffView.state.noUncommittedChanges')
                            : comparisonMode === 'committed'
                                ? t('diffView.state.noCommittedChanges', { branch: worktreeComparison.baseBranch })
                                : t('diffView.state.noCombinedChanges', { branch: worktreeComparison.baseBranch })
                        : renderedDiffScope === 'turn'
                            ? t('diffView.state.noLastTurnChanges')
                            : renderedDiffScope === 'branch-range' && branchBase
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-background">
            <div className="@container/diff-toolbar flex min-w-0 items-center gap-2 px-3 py-2 bg-background">
                {!isMobile && (
                    renderedDiffScope !== 'all' ? (
                        <ChangeScopeSelector
                            scope={renderedDiffScope}
                            worktreeAvailable={worktreeComparisonAvailable}
                            worktreeCount={worktreeFileCount}
                            worktreeBaseBranch={worktreeComparison?.available ? worktreeComparison.baseBranch : null}
                            ...
                            onScopeChange={(scope) => {
                                ...
                                if (scope !== 'branch-range') onDiffScopeChange?.(scope);
                            }}
                        />
                    ) : (
                ...
                {worktreeComparisonSelected && worktreeComparison?.available !== false ? (
                    <Select<GitWorktreeComparisonMode>
                        value={comparisonMode}
                        onValueChange={(mode) => setComparisonMode(mode)}
                    >
                        <SelectTrigger
                            size="sm"
                            className="max-w-40 min-w-0"
                            aria-label={t('diffView.comparison.mode.selectorAria')}
                        >
                            <SelectValue>{t(getComparisonModeLabelKey(comparisonMode))}</SelectValue>
                        </SelectTrigger>
                        <SelectContent align="start">
                            <SelectItem value="uncommitted">{t('diffView.comparison.mode.uncommitted')}</SelectItem>
                            <SelectItem value="committed">{t('diffView.comparison.mode.committed')}</SelectItem>
                            <SelectItem value="combined">{t('diffView.comparison.mode.combined')}</SelectItem>
                        </SelectContent>
                    </Select>
                ) : null}
                    ...
                    <Button
                        ...
                        onClick={() => {
                            ...
                            requestWalkthroughSource(directory, {
                                kind: 'working-tree',
                                scope: renderedDiffScope === 'staged' || renderedDiffScope === 'working'
                                    ? renderedDiffScope
                                    : 'all',
                            });
                ...
                {changedFiles.length > 0 && !isWorktreeComparisonScope && (
                    <Tooltip>
