...
export const MobileSessionsSheet: React.FC<MobileSessionsSheetProps> = ({ open, onOpenChange, variant = 'drawer', footer }) => {
  ...
  const renderBucketSessions = (bucketKey: string, bucket: WorktreeBucket, indent: number, project?: ProjectMeta) => {
    ...
    const renderNode = (session: Session, rowIndent: number): React.ReactNode => {
      ...
      if (project?.isGitRepo && !getParentId(session) && sourceDirectory) {
        handleMoveToWorktree = () => {
          ...
          startSessionTreeWorktreeMove({
            ...
            onSuccess: (worktreePath) => {
              ...
              setWorktreeExpanded(`${project.id}::${normalizedWorktreePath}`, true);
                          ...
                          return (
                            <>
                              {rootBucket && rootBucket.sessions.length > 0
                                ? renderBucketSessions(`${node.project.id}::${rootBucket.key}`, rootBucket, PROJECT_SESSION_INDENT, node.project)
                                : null}
                              {worktreeBuckets.map((bucket) => {
                                ...
                                return (
                                  <div key={bucket.key}>
                                    ...
                                    {worktreeExpanded
                                      ? renderBucketSessions(`${node.project.id}::${bucket.key}`, bucket, PROJECT_SESSION_INDENT, node.project)
                                      : null}
                                  </div>
                                );
                              })}
