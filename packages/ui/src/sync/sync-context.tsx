...
import { appendNotification, markSessionViewed } from "./notification-store"
    ...
    void (async () => {
      try {
        const statuses = await runBackgroundNetworkTask(() =>
          resyncDirectorySessionStatuses(directory, store, [sessionID], "monotonic", false))
        if (!statuses) return
        if (needsSnapshotAfterStatusPoll(store.getState(), sessionID, statuses.snapshot[sessionID])) {
          // A completed message cannot be an interrupted turn. Commit only its
          // status so unrelated pending candidates still require blocker recovery.
          const authoritativeStatuses = await runBackgroundNetworkTask(() =>
            resyncDirectorySessionStatuses(directory, store, [sessionID], "authoritative", false))
          if (!authoritativeStatuses?.authority || !authoritativeStatuses.baseline) return
          const current = store.getState()
          if (
            current.sessionStatusReconciliationAuthority !== authoritativeStatuses.authority
            || current.session_status !== authoritativeStatuses.baseline
          ) return
          applySessionStatusSnapshot(
            store,
            authoritativeStatuses.snapshot,
            [sessionID],
            "authoritative",
          )
          applyGlobalSessionStatusSnapshot(
...
export function handleEvent(
  ...
  // Inbox unread state is server-owned; this local path retains error details
  // for the transcript and diagnostics only.
  if (payload.type === "session.error") {
    const props = payload.properties as { sessionID?: string; error?: OpenCodeSessionErrorPayload }
    const sessionID = props.sessionID
    const errorSummary = summarizeOpenCodeError(props.error)
    if (sessionID) {
      recordSessionError({ sessionId: sessionID, directory: resolvedDirectory ?? null, ...errorSummary })
      const session = getDirectoryEventState(store, batch).session.find((candidate) => candidate.id === sessionID)
