...
export const applyGlobalSessionStatusEvents = (directory: string, payloads: readonly Event[]): void => {
  ...
  let interruptedIds: Set<string> | null = null;
  ...
  const clearInterrupted = (sessionId: string): void => {
    if (!(interruptedIds ?? state.interruptedIds).has(sessionId)) return;
    interruptedIds ??= new Set(state.interruptedIds);
  ...
  for (const payload of payloads) {
    if (payload.type === 'session.status') {
      ...
      clearInterrupted(props.sessionID);
  ...
  if (statusById || interruptedIds) {
    useGlobalSessionStatusStore.setState({
      statusById: statusById ?? state.statusById,
      interruptedIds: interruptedIds ?? state.interruptedIds,
