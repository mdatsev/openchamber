...
| `session-ui-store.ts` | OpenCode session selection, draft lifecycle, abort prompts, worktree metadata, SDK-facing action entrypoints | App UI state |
| `useChatSelectionStore.ts` | Harness-neutral visible identity projected by the active chat adapter for rows and headers | App UI state |
...
Cross-directory selectors subscribe to the narrow child-store field they aggregate. Session aggregation listens to `state.session`. Live busy/retry state is also maintained in `global-session-status.ts`, where each row subscribes to one session ID instead of scanning every child store. Events update the index incrementally; authoritative per-directory status snapshots seed it, clear sessions omitted as idle, and reconcile missed events. Sidebar pending-question attention reads authoritative `state.question`: visible rows subscribe by session, while collapsed aggregates subscribe only to question-map reference changes. A pending question displays as a question icon ahead of a busy/retry spinner; ordinary unread activity remains a dot. Unrelated streaming events such as `message.part.delta` must not trigger global session/status or question scans.
