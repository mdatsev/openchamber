...
A successful authoritative session-status request records when that request began. If a trailing unfinished assistant message predates that snapshot and the snapshot omits its session from the active set, the chat classifies the turn as unexpectedly interrupted instead of using historical message state to show live activity. That classification is published into a session-keyed marker so the matching sidebar row can show a static warning without subscribing every row to message history; resumed live activity, terminal materialization while viewed, deletion, and runtime switching clear it. Failed status requests record no authority, so the narrow pending-assistant fallback remains available until live status arrives.
