...
A successful authoritative session-status request records when that request began and which session IDs its active set contained. If a trailing unfinished assistant message predates that snapshot and the snapshot omits its session from the active set, the chat classifies the turn as unexpectedly interrupted instead of using historical message state to show live activity. Snapshot membership remains authoritative even when the directory store also holds an explicit idle status from reconciliation or a later event. That classification is published into a session-keyed marker so the matching sidebar row can show a static warning without subscribing every row to message history; resumed live activity, terminal materialization while viewed, deletion, and runtime switching clear it. Failed status requests record no authority, so the narrow pending-assistant fallback remains available until live status arrives. This interruption classification is independent of activity-timing persistence: a snapshot may settle the current page's live counter, but does not by itself expire a persisted activity start.

When a session is authoritatively settled — `session.idle`/`session.error` event, or an authoritative status snapshot that lowers a previously busy session — and the trailing assistant message is still *unfinished* (`time.completed` missing) with active tool parts and no pending question or permission anywhere in the session subtree, the turn is treated as interrupted (managed OpenCode process died mid-turn; the server never finalizes the parts, see openchamber#2577 / anomalyco/opencode#19023). The active parts are finalized locally as `error`/`Interrupted` with an end time, so tool timers stop and cards render the error state. The finalization is gated on an explicit idle status; absent status is unknown and is never judged. It never applies while the session is busy, including while the session or a hidden descendant awaits a question or permission. A later terminal event or refresh supersedes the local error, while a stale `running` refresh cannot regress it.
