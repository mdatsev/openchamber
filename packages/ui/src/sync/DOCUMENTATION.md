...
When a session is authoritatively settled — `session.idle`/`session.error` event, or an authoritative status snapshot that omits it from the active set — and the trailing assistant message is still *unfinished* (`time.completed` missing) with active tool parts and no pending question or permission anywhere in the session subtree, the turn is treated as interrupted (managed OpenCode process died mid-turn; the server never finalizes the parts, see openchamber#2577 / anomalyco/opencode#19023). Status-snapshot reconciliation first recovers authoritative questions and permissions; failed recovery leaves blocker authority unknown and skips finalization. The active parts are finalized locally as `error`/`Interrupted` with an end time, so tool timers stop and cards render the error state. Finalization requires either an explicit idle status or a blocker-authorized inactive snapshot that postdates the turn; absent status without that snapshot authority is unknown and is never judged. It never applies while the session is busy, including while the session or a hidden descendant awaits a question or permission. A later terminal event or refresh supersedes the local error, while a stale `running` refresh cannot regress it.
