...
| `useGlobalSessionsStore.ts` | Global active/archived entities plus root, parent/child, and directory indexes, compatibility lists, and the session ID lookup used for archived-session routing | All opened project/worktree session lists |
| `useSessionInboxStore.ts` | Server-backed unread completion tokens and pinned-session metadata | Active runtime, keyed by directory and session ID |
...
Cross-directory selectors subscribe to the narrow child-store field they aggregate. Session aggregation listens to `state.session`. Live busy/retry state is also maintained in `global-session-status.ts`, where each row subscribes to one session ID instead of scanning every child store. Events update the index incrementally; authoritative per-directory status snapshots seed it, clear sessions omitted as idle, and reconcile missed events. Sidebar pending-question attention reads authoritative `state.question`: visible rows scope requests to the row's full subagent subtree, while collapsed aggregates subscribe only to question-map reference changes. A descendant subagent question therefore displays as input needed on its visible parent instead of leaving the parent activity indicator active. A pending question displays as a question icon ahead of the busy/retry dot and elapsed duration. Ordinary unread completion state comes from the server-backed session inbox and remains lower priority than question, live activity, and persistent error presentation. Unrelated streaming events such as `message.part.delta` must not trigger global session/status, question, or inbox scans.
...
Incomplete-session materialization is deduplicated by runtime, directory, and session for the full cooldown window, including after a fast success or failure. A settled-running-tool recovery may supersede a different request in that window so an earlier pre-settlement refresh cannot consume the only terminal recovery signal. Deferred recovery is dropped if its captured runtime is no longer active. If recovery requests a tail refresh while an older load is in flight, one refresh runs after that load instead of losing the newer authority demand. Completion retains the cooldown marker until expiry, and an older completion cannot clear a newer request marker. Recovery starts after the current ordered event batch and rechecks whether local state already contains the requested entity before starting HTTP. A local `Interrupted` finalization still counts as needing its queued settled-tool recovery: authoritative terminal tool/message state replaces the local error and clears the sidebar interruption marker, while a stale running snapshot cannot regress it. An explicit empty part bucket is authoritative fetched-empty state, not a missing snapshot. This prevents repeated orphan/missing-part events from creating message-tail and status request storms while preserving later recovery.
...
A successful authoritative session-status request records when that request began and which session IDs its active set contained only after authoritative pending-question and permission recovery succeeds. Reconnect recovery covers each candidate's full descendant subtree. A blocking-request list replaces its captured buckets only when the relevant state container is still the request-start authority; notifications and VS Code auto-accept likewise run only after that authoritative commit. An intervening ask/reply event preserves newer state and defers the combined status commit. If either blocking-request load fails or loses authority, blocker authority remains unknown: sync preserves the previous local and global status, snapshot authority, tools, and interruption markers, then marks the directory for another combined reconciliation on the next five-second status watchdog poll. Bootstrap uses the same commit boundary and pending retry signal. If a trailing unfinished assistant message predates an authoritative inactive snapshot, the chat classifies the turn as unexpectedly interrupted instead of using historical message state to show live activity; an ordinary completed or failed tool does not suppress that turn-level classification. Snapshot membership remains authoritative even when the directory store also holds an explicit idle status from reconciliation or a later event. A locally finalized `Interrupted` tool remains interruption evidence, while a normally completed assistant message remains settled. The classification is published into a session-keyed marker so the matching sidebar row can show a static warning without subscribing every row to message history; resumed live activity, terminal materialization while viewed, deletion, and runtime switching clear it. Failed status requests record no authority, so the narrow pending-assistant fallback remains available until live status arrives. This interruption classification is independent of activity-timing persistence: a snapshot may settle the current page's live counter, but does not by itself expire a persisted activity start.

When a session is authoritatively settled — `session.idle`/`session.error` event, or an authoritative status snapshot that omits it from the active set — and the trailing assistant message is still *unfinished* (`time.completed` missing) with no pending question or permission anywhere in the session subtree, the turn is treated as interrupted (managed OpenCode process died mid-turn; the server never finalizes the message or parts, see openchamber#2577 / anomalyco/opencode#19023). Status-snapshot reconciliation first recovers authoritative questions and permissions; failed recovery leaves blocker authority unknown and skips finalization. The unfinished assistant message is completed locally with `MessageAbortedError`, including text-only turns and turns whose tools had already finished. Any active parts are finalized as `error`/`Interrupted` with an end time. Finalization requires either an explicit idle status or a blocker-authorized inactive snapshot that postdates the turn; absent status without that snapshot authority is unknown and is never judged. It never applies while the session is busy, including while the session or a hidden descendant awaits a question or permission. A later terminal event or refresh supersedes the local error, while a stale unfinished refresh cannot regress the locally finalized message or parts.
...
9. Before routing a prompt, slash command, or shell input to an existing session that the global cache authoritatively classifies as archived, restore it and wait for server confirmation. A failed or stale-runtime restore cancels the send before optimistic insertion or dispatch; never write `time.archived = 0` speculatively for active sessions because the server's active-only list filter excludes every row where that field is present.
