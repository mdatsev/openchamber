          ...
          if (version < 14) {
            ...
            if (byDirectory && typeof byDirectory === 'object') {
              for (const directoryState of Object.values(byDirectory as Record<string, unknown>)) {
                ...
                const previousActiveTabId = typeof entry.activeTabId === 'string' ? entry.activeTabId : null;
                let migratedActiveTabId: string | null = null;
                ...
                for (const rawTab of entry.tabs as Array<unknown>) {
                  ...
                  if (tab.id === previousActiveTabId) migratedActiveTabId = id;
                ...
                if (previousActiveTabId?.startsWith('preview')) {
                  const nextActive = migrated.find((tab) => typeof tab.id === 'string');
                  entry.activeTabId = migratedActiveTabId
                    ?? (nextActive && typeof nextActive.id === 'string' ? nextActive.id : null);
                }
              }
            }
          }
