...
OpenCode chat composer drafts, confirmed mentions, inline-comment drafts, and pinned sessions use the same runtime/directory/session ownership rule. Prime transcript drafts share the bounded chat-draft envelope but use a tagged runtime/harness/session key, so equal OpenCode and Prime IDs cannot collide and Prime draft ownership does not change with directory presentation metadata. Chat drafts notify mounted composers when authoritative deletion clears their identity, preventing unmount autosave from resurrecting deleted text; an accepted Prime send may conditionally clear only the exact captured text so a late completion cannot erase a newer remounted draft. Inline drafts enforce per-session, global-session, and serialized-byte bounds. Pins retain every valid composite key across runtimes without silent age/count eviction and are never pruned from the first startup list. Confirmed local deletion and routed deletion events clear immediately; after an authoritative baseline exists, a later complete omission also cleans persisted state. Ambiguous session-only legacy drafts and pins are not claimed.
