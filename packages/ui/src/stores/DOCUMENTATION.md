...
Chat composer drafts, confirmed mentions, inline-comment drafts, and session inbox records use the same runtime/directory/session ownership rule. Chat drafts use a bounded shared envelope and notify mounted composers when authoritative deletion clears their identity, preventing unmount autosave from resurrecting deleted text. Inline drafts enforce per-session, global-session, and serialized-byte bounds. `useSessionInboxStore.ts` projects the active server's unread completion tokens and pins, rejects stale record revisions, preserves prior state on failed hydration, and retries hydration when a concurrent mutation invalidates its baseline. A successful snapshot replaces active-runtime inbox authority without touching inactive runtime pin mirrors. Existing `oc.sessions.pinned.v2` entries merge into the server once per runtime, then remain only a browser mirror. Inbox records have no silent age/count eviction and authoritative deletion clears the exact directory/session tuple.
