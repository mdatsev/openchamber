...
- Root chat rows can move an idle chat and its sub-chats into a generated worktree from their swipe actions. Session history always moves; when the source checkout has uncommitted changes, the user chooses whether to move those changes or leave them in place, matching the web and Desktop action.
