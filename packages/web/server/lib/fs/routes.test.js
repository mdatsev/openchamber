...
describe('fs read', () => {
  ...
  it('reads canonical paths outside the workspace that no workspace symlink reaches', async () => {
    const fsPromises = {
      realpath: vi.fn(async (targetPath) => targetPath),
      stat: vi.fn(async () => ({ isFile: () => true, size: 6 })),
      readFile: vi.fn(async () => 'secret'),
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('secret');
    expect(fsPromises.realpath).toHaveBeenCalledWith('/outside/shared/file.txt');
    expect(fsPromises.stat).toHaveBeenCalledWith('/outside/shared/file.txt');
    expect(fsPromises.readFile).toHaveBeenCalledWith('/outside/shared/file.txt', 'utf8');
  });
  ...
  it('canonicalizes outside-workspace traversal before following filesystem symlinks', async () => {
    const fsPromises = {
      realpath: vi.fn(async (targetPath) => {
        if (targetPath === '/etc/passwd') return '/canonical/etc/passwd';
        return targetPath;
      }),
      stat: vi.fn(async () => ({ isFile: () => true, size: 6 })),
      readFile: vi.fn(async () => 'secret'),
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('secret');
    expect(fsPromises.realpath).toHaveBeenCalledWith('/etc/passwd');
    expect(fsPromises.stat).toHaveBeenCalledWith('/canonical/etc/passwd');
    expect(fsPromises.readFile).toHaveBeenCalledWith('/canonical/etc/passwd', 'utf8');
  });
});
describe('fs reveal', () => {
  it.each([
...
describe('fs stat directory scope (issue 3019)', () => {
  ...
  it('allows an authenticated stat outside the last active workspace', async () => {
