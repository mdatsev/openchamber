...
- `getWorktreeComparison(directory, { mode, includePatches, contextLines })`: Compare a linked worktree using `uncommitted`, `committed`, or `combined` mode. Protocol v2 returns committed, staged, unstaged, and untracked layers; the summary form omits layer file patches. Exposed as `GET /api/git/worktree-comparison`. The route defaults an omitted `mode` to `combined` for older clients.
...
- Protocol v2 responses include `protocolVersion: 2`, the requested mode, and `primaryWorktree` for refresh correlation. A request for the primary worktree returns `{ protocolVersion: 2, available: false, reason: 'primary-worktree', mode }`.
- `uncommitted` does not require primary/shared ancestry. It records primary branch/HEAD when available, then independently returns a staged layer (`HEAD` to index), unstaged layer (index to working tree), and untracked layer (empty tree to file). Staged and unstaged changes remain visible even when their net `HEAD`-to-worktree content cancels.
- `committed` computes `mergeBase` from the request-time primary `baseHead` and linked `head`, then returns a committed layer from that merge base to linked `HEAD`. It excludes staged, unstaged, and untracked changes as well as commits that exist only on the primary branch.
- `combined` returns the committed layer plus the same three uncommitted layers. It is not flattened into one net patch: committed and dirty changes remain visible even when later dirty edits reverse committed content, and the same path may intentionally appear in multiple layers.
- Graph ancestry is authoritative. Cherry-pick and squash equivalence are intentionally unsupported: a patch-equivalent commit that is not reachable from the primary branch remains a committed comparison change. The API does not run `git cherry` or infer integration from patch identity.
- Every successful v2 response includes the requested `mode`, request-time primary branch/HEAD when available, linked `head`, `mergeBase` when ancestry is required, and each layer's exact `base` and `target`.
- An omitted-mode request is treated as a legacy combined request. It also returns top-level net `files`, `diffBase`, and former unintegrated-count fields so pre-v2 clients continue to render instead of failing. New clients validate `protocolVersion: 2`; they normalize older combined responses to one `legacy-combined` layer, but reject older-server responses for committed-only or uncommitted-only mode rather than spinning or showing clean state.
- The comparison is read-only. It never creates OpenCode project metadata, stages, unstages, resets, checks out, or otherwise mutates either worktree.
