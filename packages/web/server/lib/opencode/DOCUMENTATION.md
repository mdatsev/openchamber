...
- `packages/web/server/lib/prime-agent/routes.js`: bounded Prime Agent catalog, transcript, explicit attachment, lifecycle, and mutation routes.
...
Prime Agent routes are also registered before the generic proxy. `GET /api/prime/sessions` distinguishes a missing Prime state directory from an authoritative empty catalog and isolates malformed files. `GET /api/prime/sessions/:sessionId/transcript` resolves IDs only inside the configured Prime session directory, rejects symlinks and oversized or malformed input, and returns the current parent-linked branch without mutating Prime storage or daemon state. Interactive attachment uses the origin-gated `POST /api/prime/sessions/:sessionId/attach`; create, prompt, abort, and reconnect are separate origin-gated mutations.
