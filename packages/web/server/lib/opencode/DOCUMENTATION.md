...
- `packages/web/server/lib/prime/`: authenticated passive Prime history plus explicit daemon-v7 activation, cached snapshots/model catalogs, live SSE lifecycle routes, and fenced existing-session mutations.
...
Prime routes are registered by this feature runtime after the shared `/api` authentication middleware and before the generic OpenCode proxy. Phase 2 status/catalog/transcript/context GETs remain filesystem-only; only origin-checked activation and fenced mutation POSTs enter the retained daemon-v7 runtime manager. Mutation bodies use a route-local 300 KiB JSON bound and fixed parser errors. Cached snapshots and SSE reads never activate. The feature runtime owns service disposal so graceful shutdown disables reconnect, detaches, and closes Prime sockets before HTTP close; the server close hook provides an idempotent fallback for other restart/close paths. `/api/prime/**` has a dedicated terminator so unknown routes never fall through upstream. See `packages/web/server/lib/prime/DOCUMENTATION.md` for the filesystem, snapshot, reconnect, mutation, normalization, and runtime-parity contracts.
