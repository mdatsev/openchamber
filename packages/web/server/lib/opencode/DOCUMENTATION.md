...
- `registerOpenChamberRoutes(app, dependencies)`: registers OpenChamber endpoints:
  ...
  - `POST /api/openchamber/fork/update`
    ...
    - Keep the `/api/openchamber/fork` route family in the common JSON body-parser allowlist; the generic `/api` branch intentionally skips parsing, and losing `mode: rebuild-current` silently turns a requested rebuild into a no-op when the checkout is already current.
